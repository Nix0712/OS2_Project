...
void init_raid_device() {
  ...
  raid_device.is_init =
      -1; // Could change to array of init vals, for one use of disks metadata
...
int formatOneDisk(uint64 disk_num, uchar *nullData) {
  ...
  for (int j = 0; j < 128 * 1024 / BSIZE; j++)
...
int rw_block(struct RAIDSuperblock *currMetadata, uint64 block_num,
             uint64 p_buff, int isRead) {
  ...
  case RAID0_1:
    ...
    disk_num = (block_num % currMetadata->num_of_disks) + 1;
    blkc_num =
        ((block_num + currMetadata->num_of_disks) / currMetadata->num_of_disks);
  ...
  case RAID4:
    ...
    break;
  case RAID5:
    //TODO: reachack calcutation, somting is not correct
    ...
    disk_num =
        ((parrity_index + stripe_offset) % (currMetadata->num_of_disks - 1)) +
        1;
...
int raid_read_block(uint64 block_num, uint64 buffAddr) {
  ...
  // TODO: chage to read from the raid_device on initalization and not from UNHEALTY
  uchar data[BSIZE];
  read_block(1, 0, data);
